...
### Tests
- ✅ `tests/test_phase7.py` (14 new): timeline storage round-trip + retention cap (tmp path, artifact
  untouched) + no-wall-clock; attribution contract (sources, per-source cells ⊆ margin, rates ∈ [0,1],
  margins sum == caught) + determinism across perturbed global RNG + provenance via `combine_matrices`;
  sanctions `build_watch_list` canonical-order regression; the three panel endpoints
  (`/api/timeline`, `/api/attribution`, `/api/rl-stretch`) defined-schema JSON (client without init).

### Gate
✅ Full suite green (230 passed, +14 new Phase 7 tests) + all four panels render on the Dashboard
(blind-spot timeline, RL kill-criterion result, attribution matrix, fitted-weights transparency):
1. **Timeline** — `src/feedback/timeline.py` (append-only, capped 30, no wall-clock) + deterministic
   `scripts/timeline_eval.py` (seed-pinned RNGs) → `artifacts/feedback_timeline.json` byte-identical
   across re-runs: 4 cycles, idx0 0.375→1.0 (relational camouflage), idx1 0.875→1.0, idx2/3 clean.
   `main.py` `/api/demo/run` appends each live cycle to the same file; `/api/timeline` serves it.
2. **RL panel** — `/api/rl-stretch` serves the real `rl_stretch` block (honest semantics:
   `honest_negative = not shipped`); panel shows DO NOT SHIP / SHIPPED + criterion + registry metrics.
3. **Attribution** — `src/eval/attribution.py` (`build_attribution_matrix`, `combine_matrices`,
   `mechanism_label`) + `src/investigate/case_manager.py::evidence_flags_for` (same fixtures/sanctions
   agent as the case path) + deterministic `scripts/attribution_eval.py` → `artifacts/attribution.json`
   fp `237b553e4a42795e`: rule_compiler 33 · shadow_pgd 7 · llm_strategist 12 · genetic 8 · other 4
   (untagged held-out A5 rows) · protocol_structural 0 caught — honest zero (agentic world). OOD
   llm_strategist rows are re-tagged `mechanism="llm_strategist"` after compile (compiler overwrites).
4. **Fitted weights** — Phase 4 panel (already on Dashboard), weights artifact regenerated byte-identical.

### Hygiene fix (exposed by the determinism gate)
`build_watch_list` (src/investigate/sanctions.py) drew its seeded RNG while iterating a SET, so hit
membership depended on per-process hash order (PYTHONHASHSEED). Now sorts BEFORE filtering — canonical
order, byte-identical watch lists across processes. This previously-fragile seam now has a regression test.
