# Project Prometheus — Work Plan 

This is not a re-statement of the plan — every item below was verified by reading the actual source.
...
built**. The project is materially ahead of its own plan in
...
| Attack Compiler, funding-aware entity selection | `src/attack/compiler.py` |  fix: tiered funded-pool selection (100/50/20%) |
...
| Test coverage | `tests/` (13 files) | investigator, fidelity, mechanisms, shadow, blue, phase integrity all covered |
...
- **Priority: do this first** — a technical judge who reads both modules will notice the disconnect immediately.
...
- **Root cause** (traced through code, not guessed):
  ...
  - funding-aware selection reads *live* balances correctly, but has no defense against **cross-attack-type depletion within a single eval pass** — it can only fall back to unconstrained selection when the funded pool is empty, which then gets clamped by the never-overdraft rule → too few fraud rows → fail-loud gate trips.
...
## 3. Standout ideas which should be implemented
...
## 6. GROUND-BREAKING ADDITIONS WHICH HAD TO BE THERE IN THE PROJECT
...
5. **Report it like the paper does**: attack-success-rate before/after the
   ...
   anyone else attempt.
-  for a scoped version (2-3 of the RC classes done
...
- if using an existing lightweight TabDDPM
...
1. §2.1 — wire OSINT/campaign evidence into the score 
2. §2.3 — ring-fence A5/A4 funding + add diagnostics 
3. **§6.1 — Agentic-Commerce Protocol Attacks (RC-1 + RC-4 minimum) + PCAT-style defense**  — **highest differentiation-per-hour item in this whole document**
4. §2.2 — fit score weights 
5. §5 — live SSE/WebSocket visualization 
6. §6.2 — diffusion-model fidelity critic upgrade 
7. §3 — standout panels (RL negative result, mechanism attribution, fitted-weights transparency, protocol-attack before/after) 
8. §2.4 — hygiene pass
